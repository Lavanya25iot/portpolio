...
DATABASE_NAME = os.path.join(os.path.dirname(__file__), 'portfolio.db')

def get_local_timestamp():
    """Return local timestamp in SQLite friendly format"""
...
def add_contact(name, email, message):
    ...
    try:
        ...
        cursor.execute(
            ...
            (name, email, message, get_local_timestamp())
        )
        conn.commit()
        conn.close()
        return True
